                    ...
                    <div class="flex flex-col md:flex-row">
                        <div class="md:w-1/2 pr-4">
                            @if($event->foto_poster)
                                <img src="{{ Storage::url($event->foto_poster) }}" alt="{{ $event->nama }}" class="w-full rounded-lg shadow-md mb-4">
                            @else
                                <div class="w-full h-64 bg-gray-300 flex items-center justify-center rounded-lg mb-4">
                                    <span class="text-gray-500">No Image Available</span>
                                </div>
                            @endif
                        </div>
                        <div class="md:w-1/2">
                            <h1 class="text-3xl font-bold mb-4">{{ $event->nama }}</h1>
                            <div class="space-y-3">
                                <p class="flex items-center">
                                    ...
                                    <span><strong>Date:</strong> {{ \Carbon\Carbon::parse($event->tanggal)->format('F d, Y') }}</span>
                                </p>
                                <p class="flex items-center">
                                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"></path></svg>
                                    <span><strong>Location:</strong> {{ $event->lokasi }}</span>
                                </p>
                                <div class="flex items-start">
                                    <svg class="w-5 h-5 mr-2 mt-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 10h16M4 14h16M4 18h16"></path></svg>
                                    <div>
                                        <strong>Description:</strong>
                                        <p class="mt-2">{{ $event->deskripsi }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
